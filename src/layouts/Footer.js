...
const NON_CLICKABLE = ['About Us', 'Case Studies', 'Careers', 'Contact Us'];
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {FOOTER_LINKS.map(({ label, url }) => {
                    if (NON_CLICKABLE.includes(label)) {
                      return (
                        <li key={label}>
                  ...
                  {/*
                  // Original clickable mapping:
                  {FOOTER_LINKS.map(({ label, url }) => (
                    <li key={label}>
                      <Link href={url} className="text-gray-600 hover:text-black transition-colors">
                        {label}
                      </Link>
                    </li>
                  ))}
