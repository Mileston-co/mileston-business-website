...
import { faArrowRight, faChartLine, faShieldHalved, faGlobe } from '@fortawesome/free-solid-svg-icons';
...
const Stablecoin = () => {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(exchangeRef.current,
      { y: 20, opacity: 0 },
      { 
        y: 0, 
        opacity: 1, 
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-center">
            <FontAwesomeIcon icon={faArrowRight} className="text-gray-400 exchange-arrow" />
          </div>
            ...
            <div className="flex gap-4">
              ...
              >
                {stablecoins.map(coin => (
                  <option key={coin.code} value={coin.code}>{coin.code}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <button
            className="w-full py-3 font-medium text-white transition-transform bg-black rounded-lg active:scale-[0.99]"
          >
            Exchange Now
          </button>
        </div>
